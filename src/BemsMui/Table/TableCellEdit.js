...
import TextFieldReadOnly from 'BemsMui/TextFieldReadOnly';
  ...
  return (
    <td className={className} onClick={handleClick.bind(null, onRequestOpen)} {...other}>
      {React.Children.map(children, (child, idx) => {
        ...
        return <TextFieldReadOnly placeholder={value || placeholder} />;
      })}

      <Popover open={isOpen} anchorEl={openCell} anchorOrigin={anchorOrigin} transformOrigin={targetOrigin} onRequestClose={onRequestClose}>
        {children}
      </Popover>
    </td>
  );
